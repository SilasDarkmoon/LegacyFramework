	...
	switch(obj.type){
		case JSONObject.Type.OBJECT:
			for(int i = 0; i < obj.list.Count; i++){
				...
				if (GLog.IsLogInfoEnabled) GLog.LogInfo(key);
				accessData(j);
			}
			break;
		...
		case JSONObject.Type.STRING:
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(obj.str);
			break;
		case JSONObject.Type.NUMBER:
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(obj.n);
			break;
		case JSONObject.Type.BOOL:
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(obj.b);
			break;
		case JSONObject.Type.NULL:
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("NULL");
			break;
...
list.GetField("hits", delegate(JSONObject hits) {
	hits.GetField("hits", delegate(JSONObject hits2) {
		foreach (JSONObject gameSession in hits2.list) {
			if (GLog.IsLogInfoEnabled) GLog.LogInfo(gameSession);
		}
	});
}, delegate(string name) {	//"name" will be equal to the name of the missing field.  In this case, "hits"
	if (GLog.IsLogWarningEnabled) GLog.LogWarning("no game sessions");
});
</syntaxhighlight>
...
if (GLog.IsLogInfoEnabled) GLog.LogInfo(arr[2].n);		//Should ouptut "3"
</syntaxhighlight>

...
//#endif
	...
	public static JSONObject Create() {
		...
		while(result == null && releaseQueue.Count > 0) {
			...
			if(result == null)
				if (GLog.IsLogInfoEnabled) GLog.LogInfo("wtf " + releaseQueue.Count);
			else if(result.list != null)
				if (GLog.IsLogInfoEnabled) GLog.LogInfo("wtflist " + result.list.Count);
#endif
		}
		if(result != null)
			return result;
	...
	void Parse(string str, int maxDepth = -2, bool storeExcessLevels = false, bool strict = false) {
		if(!string.IsNullOrEmpty(str)) {
			...
			if(strict) {
				if(str[0] != '[' && str[0] != '{') {
					type = Type.NULL;
					if (GLog.IsLogWarningEnabled) GLog.LogWarning("Improper (strict) JSON formatting.  First character must be [ or {");
					return;
				}
			}
			if(str.Length > 0)
...
//#endif
								...
								type = Type.NULL;
								if (GLog.IsLogWarningEnabled) GLog.LogWarning("improper JSON formatting:" + str);
							}
							return;
					}
					string propName = "";
	...
	static void MergeRecur(JSONObject left, JSONObject right) {
		if(left.type == Type.NULL)
			...
			if(right.Count > left.Count) {
				if(GLog.IsLogErrorEnabled) GLog.LogError("Cannot merge arrays when right object has more elements");
				return;
			}
			for(int i = 0; i < right.list.Count; i++) {
...
#pragma warning restore 219
	...
	IEnumerable StringifyAsync(int depth, StringBuilder builder, bool pretty = false) {	//Convert the JSONObject into a string
		...
		if(depth++ > MAX_DEPTH) {
			if (GLog.IsLogInfoEnabled) GLog.LogInfo("reached max depth!");
			yield break;
		}
...
#if(PRETTY)
					if(pretty)
                        builder.Length -= (System.Environment.NewLine.Length + 1);
                    else
#endif
					builder.Length--;
				}
...
#if(PRETTY)
					if(pretty)
						builder.AppendLine(); //for a bit more readability
#endif
					for(int i = 0; i < list.Count; i++) {
						if(list[i]) {
#if(PRETTY)
...
//#endif
	...
	public Dictionary<string, string> ToDictionary() {
		...
		if (GLog.IsLogWarningEnabled) GLog.LogWarning("Tried to turn non-Object JSONObject into a dictionary");
		return null;
	}

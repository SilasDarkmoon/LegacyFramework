...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        private static bool CheckIsShowLog()
        {
#if DEVELOPMENT_BUILD || UNITY_EDITOR
            return Application.isPlaying && _isShowLog;
#else
            return false;
#endif
        }
        ...
        public bool TryDelayWrapLoader(RealStatePtr L, Type type)
        {
            ...
            {
                if (CheckIsShowLog()) Profiler.BeginSample("wrap注册 => " + type);
                delayWrap.Remove(type);
                loader(L);
                if (CheckIsShowLog())
                {
                    Profiler.EndSample();
                    GLog.LogError("wrap注册 => " + type + " 耗时 => " + (Time.realtimeSinceStartup - t).ToString("F5"));
                }
            }
            else
            {
                if (CheckIsShowLog()) Profiler.BeginSample("动态反射 =============================> " + type);
#if !GEN_CODE_MINIMIZE && !ENABLE_IL2CPP && (UNITY_EDITOR || XLUA_GENERAL) && NET_4_6 && !FORCE_REFLECTION
                ...
                {
                    Profiler.EndSample();
                    GLog.LogError("动态反射 =============================> " + type + " 耗时 => " + (Time.realtimeSinceStartup - t).ToString("F5"));
                }
#if NOT_GEN_WARNING
...
#if (UNITY_EDITOR || XLUA_GENERAL) && NET_4_6
        ...
        private object getCsObj(RealStatePtr L, int index, int udata)
        {
            ...
            {
#if !UNITY_5 && !XLUA_GENERAL && !UNITY_2017 && !UNITY_2018

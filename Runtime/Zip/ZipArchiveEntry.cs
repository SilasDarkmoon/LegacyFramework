...
namespace Unity.IO.Compression
{
 ...
 || _archive._forceZip64
            ...
            Int32 bigExtraFieldLength = (zip64Needed ? (Int32)zip64ExtraField.TotalSize : 0)
            ...
            {
                extraFieldLength = zip64Needed ? zip64ExtraField.TotalSize : (UInt16)0;
                _cdUnknownExtraFields = null;
            }
            else
            {
 ...
 || (_archive._forceZip64 && _archive.Mode == ZipArchiveMode.Update)
            ...
            Int32 bigExtraFieldLength = (zip64Used ? (Int32)zip64ExtraField.TotalSize : 0)
                                      + (_lhUnknownExtraFields != null ? ZipGenericExtraField.TotalSize(_lhUnknownExtraFields) : 0);
            UInt16 extraFieldLength;
            if (bigExtraFieldLength > UInt16.MaxValue)
            {
                extraFieldLength = zip64Used ? zip64ExtraField.TotalSize : (UInt16)0;
                _lhUnknownExtraFields = null;
            }
            else
            {

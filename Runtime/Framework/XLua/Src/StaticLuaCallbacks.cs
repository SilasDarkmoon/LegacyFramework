...
namespace XLua
{
    ...
    public partial class StaticLuaCallbacks
    {
        ...
        {
            return PrintBase(L, UnityEngine.LogType.Log);
        }

        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        internal static int GLogInfo(RealStatePtr L)
        {
            return PrintBase(L, UnityEngine.LogType.Log);
        }

        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        internal static int GLogError(RealStatePtr L)
        {
            return PrintBase(L, UnityEngine.LogType.Error);
        }

        [MonoPInvokeCallback(typeof(LuaCSFunction))]
        internal static int GLogWarning(RealStatePtr L)
        {
            return PrintBase(L, UnityEngine.LogType.Warning);
        }
            ...
            try
            {
                ...
                switch (type)
                {
                    case UnityEngine.LogType.Error:
                        if (GLog.IsLogErrorEnabled) GLog.LogError("LUA: " + s);
                        break;
                    case UnityEngine.LogType.Warning:
                        if (GLog.IsLogWarningEnabled) GLog.LogError("LUA: " + s);
                        break;
                    default:
                        if (GLog.IsLogInfoEnabled) GLog.LogInfo("LUA: " + s);
                        break;
                }
                return 0;
            }

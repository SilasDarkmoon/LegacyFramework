    ...
    public class QRCodeGenerator
    {
        ...
        private static class ModulePlacer
        {
            ...
            public static int MaskCode(ref QRCodeData qrCode, int version, ref List<Rectangle> blockedModules)
            {
                ...
                {
                    if (pattern.Key.Length == 8 && pattern.Key.Substring(0, 7) == "Pattern")
                    {
                ...
                var patterMethod = MaskPattern.Patterns[patternName];
                for (int x = 0; x < size; x++)
                {
                    for (int y = 0; y < size; y++)
                    {
                        if (!IsBlocked(new Rectangle(x, y, 1, 1), blockedModules))
                        {
                            qrCode.ModuleMatrix[y][x] ^= patterMethod(x, y);
                        }
                    }
                }
                return Convert.ToInt32(patternName.Substring(patternName.Length - 1, 1)) - 1;
            }
            ...
            private static class MaskPattern
            {
                ...
                public static Dictionary<string, Func<int, int, bool>> Patterns = new Dictionary<string, Func<int, int, bool>>()
                {
                    {"Pattern1", Pattern1},
                    {"Pattern2", Pattern2},
                    {"Pattern3", Pattern3},
        ...
        private void CreateAlphanumEncDict()
        {
            ...
            foreach(var x in alphanumEncTable.ToList().Select((x, i) => new { Chr = x, Index = i }))
            {
